...
from breezed.ipmi import IpmiClient, IpmiError, Runner
...
def make_client(runner: FakeRunner) -> IpmiClient:
    return IpmiClient(make_settings(), runner=cast(Runner, runner))


def test_parses_sdr_fixture_to_max_cpu_temp() -> None:
    runner = FakeRunner(ok((FIXTURES / "sdr_temperature.txt").read_text()))
    assert make_client(runner).read_max_cpu_temp() == TempC(58)
...
def test_nonzero_exit_code_raises_ipmi_error_with_stderr_snippet() -> None:
    runner = FakeRunner(
        SimpleNamespace(returncode=1, stdout="", stderr="unable to establish IPMI v2 connection\n")
    )
    with pytest.raises(
...
def test_commands_emit_exact_argv() -> None:
    ...
    auto_runner, manual_runner, speed_runner = (
        FakeRunner(ok("ok\n")),
        FakeRunner(ok("ok\n")),
        FakeRunner(ok("ok\n")),
    )
